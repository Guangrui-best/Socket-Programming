...
int main(int argc, char *argv[]){
    ...
    hosp_addr.sin_port = htons(32073);
    ...
    cout << "Hospital C is up and running using UDP on port 32073." << endl;
    ...
    while (1){
        ...
        if (strcmp(recvbuff, "C") == 0){
            ...
            cout << "Hospital C has been assigned to a client, occupation is updated to "
                 << hospital.occupancy << ", availabilty is updated to " << availablity << endl;
            continue;
        }
        int location = atoi(recvbuff);
        cout << "Hospital C has received input from client at location " << location << endl;
        ...
        if (idx_map.find(location) == idx_map.end()){
            ...
            cout << "Hospital C has capacity " << hospital.capacity << ", occupation = " 
            ...
            sendto(sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&scheduler_addr, scheduler_addr_len);
            cout << "Hospital C has sent score = " << score << " and distance = " << distance
